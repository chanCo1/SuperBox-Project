...
/**
 * @description 회원가입
 * @param loginPageState 로그인 팝업창 상태값 관리 (boolean) from App.jsx
 */
  ...
  const onSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        RegexHelper.pwCheck(
          current.password,
          '8~20자리의 영문(대/소문자)+숫자+특수문자만 가능합니다.'
        );

        RegexHelper.value(current.passwordCheck, '비밀번호를 확인해주세요.');
        RegexHelper.compare(
          current.password,
          current.passwordCheck,
          '비밀번호가 일치하지 않습니다.'
        );
      ...
      try {
        ...
        await axios.post('/api/users/check', register);
